...
if __name__ == "__main__":
    onlyfiles = [f for f in listdir("../captured_data/aze0/qc_charging/capt") if isfile(join(
        "../captured_data/aze0/qc_charging/capt", f)) and f.endswith(".bin")]
    onlyfiles.sort()
    dtrows = []
    for file in onlyfiles:
        with open(f"../captured_data/aze0/qc_charging/capt/{file}", "rb") as f:
            data = f.read()
            ev_data = data[153:]

            try:
                result = parse_evinfo(ev_data, aze0=True)
